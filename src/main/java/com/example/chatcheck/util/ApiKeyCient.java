...
public class ApiKeyCient {
    ...
    public static String[] toType(String key, boolean isN) {
        if (isN) {
            ...
            key = key.replaceAll("LIU", "").replaceAll("DAW", "").replaceAll("BLT", "").replaceAll("JIE", "").replaceAll("GAO", "");
            //isN为true时，是短模型，强制1105
        } else {
            //isN为false时，是长模型，建议逆向
            key = key.replaceAll("2L", "").replaceAll("2D", "").replaceAll("2B", "").replaceAll("2J", "").replaceAll("2G", "");
        }
